...
<div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:12px; margin-bottom:20px;">
    <div>
        <h1 style="font-size:1.25rem; font-weight:800; color:#0f172a; margin:0;">{{ __('Fines') }}</h1>
        <p style="font-size:0.8rem; color:#64748b; margin:3px 0 0;">{{ __('Manage all member fines and penalties') }}</p>
    </div>
    <button onclick="openModal('modal-create')" style="{{ $btn_primary }}">
        <i class="fas fa-plus"></i> {{ __('Add Fine') }}
    </button>
</div>

<div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(180px,1fr)); gap:14px; margin-bottom:20px;">
    <x-stat-card :label="__('Total Fines')"  :value="$cur . number_format($stats['total_amount'], 2)"   icon="fas fa-triangle-exclamation" bg="#fef2f2" iconColor="#ef4444" />
    <x-stat-card :label="__('Collected')"    :value="$cur . number_format($stats['paid_amount'], 2)"    icon="fas fa-circle-check"         bg="#f0fdf4" iconColor="#16a34a" />
    <x-stat-card :label="__('Pending')"      :value="$cur . number_format($stats['pending_amount'], 2)" icon="fas fa-clock"                bg="#fefce8" iconColor="#ca8a04" />
    <x-stat-card :label="__('Total Count')"  :value="$stats['total_count']"                            icon="fas fa-list"                 bg="#f8fafc" iconColor="#64748b" />
</div>
...
<form method="GET" style="background:#fff; border-radius:12px; border:1px solid #e2e8f0; padding:14px 18px; margin-bottom:16px; display:flex; flex-wrap:wrap; gap:10px; align-items:flex-end;">
    <div style="flex:1; min-width:180px;">
        ...
        <div style="position:relative;">
            ...
            <input name="search" value="{{ request('search') }}" placeholder="{{ __('Member name or reason...') }}"
...
<div style="background:#fff; border-radius:14px; border:1px solid #e2e8f0; box-shadow:0 1px 6px rgba(0,0,0,0.05); overflow:hidden;">
    <div style="padding:14px 20px; border-bottom:1px solid #f1f5f9;">
        <span style="font-size:0.88rem; font-weight:700; color:#1e293b;">{{ __('Fine Records') }} <span style="color:#94a3b8; font-weight:400;">({{ $fines->total() }})</span></span>
    </div>
    <div style="overflow-x:auto;">
        <table style="width:100%; border-collapse:collapse; font-size:0.83rem;">
            <thead>
                <tr style="background:#f8fafc;">
                    @foreach([__('#'),__('Member'),__('Samity'),__('Reason'),__('Amount'),__('Fine Date'),__('Status'),__('Actions')] as $h)
            ...
            <tbody>
                ...
                <tr style="border-top:1px solid #f1f5f9;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background=''">
                    ...
                    <td style="padding:12px 16px; font-weight:500; color:#ef4444;">{{ $cur }}{{ number_format($f->amount, 2) }}</td>
                ...
                <tr><td colspan="8" style="padding:48px;text-align:center;color:#94a3b8;">
                    <i class="fas fa-triangle-exclamation" style="font-size:2.5rem;opacity:.2;display:block;margin-bottom:10px;"></i>{{ __('No fine records found.') }}
                </td></tr>
                @endforelse
            </tbody>
        </table>
...
{{-- ═══ CREATE MODAL ═══ --}}
<x-modal id="modal-create" :title="__('Add Fine')" icon="fas fa-triangle-exclamation">
    <form action="{{ route('fines.store') }}" method="POST">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:14px;">
            ...
            <div style="grid-column:1/-1;">
                ...
                <input name="reason" value="{{ old('reason') }}" placeholder="{{ __('Reason for fine...') }}" style="{{ $inp }}" required>
            </div>
            <div>
            ...
            <div>
                <label style="{{ $lbl }}">{{ __('Fine Date') }} *</label>
            ...
            <div style="grid-column:1/-1;">
                ...
                <select name="status" style="{{ $inp }}" required>
                    ...
                    <option value="waived"  {{ old('status') === 'waived'  ? 'selected':'' }}>{{ __('Waived') }}</option>
                </select>
            </div>
            <div style="grid-column:1/-1;">
        ...
        <div style="display:flex; gap:10px; justify-content:flex-end; margin-top:20px; padding-top:16px; border-top:1px solid #f1f5f9;">
            ...
            <button type="submit" style="{{ $btn_primary }}"><i class="fas fa-save"></i> {{ __('Save Fine') }}</button>
        </div>
    </form>
</x-modal>

{{-- ═══ EDIT MODAL ═══ --}}
<x-modal id="modal-edit" :title="__('Edit Fine')" icon="fas fa-pen-to-square">
    <form id="edit-fine-form" method="POST">
        ...
        <div style="display:flex; gap:10px; justify-content:flex-end; margin-top:20px; padding-top:16px; border-top:1px solid #f1f5f9;">
            ...
            <button type="submit" style="{{ $btn_primary }}"><i class="fas fa-save"></i> {{ __('Update Fine') }}</button>
        </div>
    </form>
</x-modal>

        ...
        <table style="width:100%; border-collapse:collapse; font-size:0.83rem;">
            <thead>
                <tr style="background:#f8fafc;">
                    @foreach([__('#'),__('Member'),__('Samity'),__('Amount'),__('Interest'),__('Duration'),__('Installment'),__('Issue Date'),__('Due Date'),__('Repaid'),__('Status'),__('Actions')] as $h)
...
{{-- ═══ CREATE MODAL ═══ --}}
<x-modal id="modal-create" :title="__('Issue New Loan')" icon="fas fa-hand-holding-dollar" width="620px">
    <form action="{{ route('loans.store') }}" method="POST">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:14px;">
            ...
            <div>
                <label style="{{ $lbl }}">{{ __('Monthly Installment') }} ({{ $cur }}) *</label>
                <input type="number" step="0.01" id="c_installment" name="monthly_installment" value="{{ old('monthly_installment') }}" placeholder="{{ __('Auto-calculated') }}" style="{{ $inp }} background:#f8fafc;" required>
            </div>
            <div>
...
{{-- ═══ EDIT MODAL ═══ --}}
<x-modal id="modal-edit" :title="__('Edit Loan')" icon="fas fa-pen-to-square" width="620px">

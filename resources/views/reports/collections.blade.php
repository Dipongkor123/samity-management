...
<div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:12px; margin-bottom:20px;">
    <div>
        <h1 style="font-size:1.25rem; font-weight:800; color:#0f172a; margin:0;">{{ __('Collection Report') }}</h1>
        <p style="font-size:0.8rem; color:#64748b; margin:3px 0 0;">{{ \Carbon\Carbon::parse($from)->format('d M Y') }} — {{ \Carbon\Carbon::parse($to)->format('d M Y') }}</p>
    </div>
...
<div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(180px,1fr)); gap:14px; margin-bottom:20px;">
    <x-stat-card :label="__('Total Collected')"  :value="$cur . number_format($totalCollected, 2)" icon="fas fa-money-bill-wave" bg="#f0fdf4" iconColor="#16a34a" />
    <x-stat-card :label="__('Transactions')"      :value="$collections->total()"                     icon="fas fa-receipt"        bg="#eff6ff" iconColor="#2563eb" />
</div>
...
<form method="GET" style="background:#fff; border-radius:12px; border:1px solid #e2e8f0; padding:14px 18px; margin-bottom:16px; display:flex; flex-wrap:wrap; gap:10px; align-items:flex-end;">
    <div>
        ...
        <input type="date" name="from" value="{{ $from }}" style="{{ $inp }} width:auto;">
    </div>
    <div>
        <label style="{{ $lbl }}">{{ __('To') }}</label>
        <input type="date" name="to" value="{{ $to }}" style="{{ $inp }} width:auto;">
    </div>
    <div>
    ...
    <div style="display:flex; gap:8px;">
        ...
        <a href="{{ route('reports.collections') }}" style="background:#f1f5f9;color:#64748b;border-radius:8px;padding:9px 14px;font-size:0.83rem;font-weight:600;text-decoration:none;display:inline-flex;align-items:center;gap:6px;">
...
<div style="background:#fff; border-radius:14px; border:1px solid #e2e8f0; box-shadow:0 1px 6px rgba(0,0,0,0.05); overflow:hidden;">
    <div style="padding:14px 20px; border-bottom:1px solid #f1f5f9;">
        <span style="font-size:0.88rem; font-weight:700; color:#1e293b;">{{ __('Collection Records') }} <span style="color:#94a3b8; font-weight:400;">({{ $collections->total() }})</span></span>
    </div>
    <div style="overflow-x:auto;">
        <table style="width:100%; border-collapse:collapse; font-size:0.83rem;">
            <thead>
                <tr style="background:#f8fafc;">
                    @foreach(['#', __('Date'), __('Member'), __('Samity'), __('Loan Amount'), __('Amount Paid'), __('Note')] as $h)
            ...
            <tbody>
                ...
                <tr style="border-top:1px solid #f1f5f9;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background=''">
                    <td style="padding:12px 16px; color:#94a3b8; font-size:0.8rem;">{{ $collections->firstItem() + $loop->index }}</td>
                    <td style="padding:12px 16px; color:#475569; white-space:nowrap;">{{ $rep->paid_date->format('d M Y') }}</td>
                    <td style="padding:12px 16px; font-weight:600; color:#1e293b;">{{ $rep->loan?->user?->name ?? '—' }}</td>
                ...
                <tr><td colspan="7" style="padding:48px;text-align:center;color:#94a3b8;">
                    <i class="fas fa-receipt" style="font-size:2.5rem;opacity:.2;display:block;margin-bottom:10px;"></i>{{ __('No collections found for this period.') }}
                </td></tr>
                @endforelse
            </tbody>
        </table>
    ...
    @if($collections->hasPages())
        <div style="padding:12px 20px; border-top:1px solid #f1f5f9;">{{ $collections->links() }}</div>
    @endif
</div>

...
<div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:12px; margin-bottom:20px;">
    <div>
        <h1 style="font-size:1.25rem; font-weight:800; color:#0f172a; margin:0;">Loan Repayments</h1>
        <p style="font-size:0.8rem; color:#64748b; margin:3px 0 0;">Track all loan installment payments</p>
    </div>
...
<div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(180px,1fr)); gap:14px; margin-bottom:20px;">
    <x-stat-card label="Total Paid"      :value="'৳' . number_format($stats['total_paid'], 2)"      icon="fas fa-rotate-left"    bg="#f0fdf4" iconColor="#16a34a" />
    <x-stat-card label="This Month"      :value="'৳' . number_format($stats['this_month'], 2)"      icon="fas fa-calendar-check" bg="#eff6ff" iconColor="#2563eb" />
    <x-stat-card label="Total Principal" :value="'৳' . number_format($stats['total_principal'], 2)" icon="fas fa-coins"          bg="#f0fdfa" iconColor="#0d9488" />
    <x-stat-card label="Total Interest"  :value="'৳' . number_format($stats['total_interest'], 2)"  icon="fas fa-percent"        bg="#faf5ff" iconColor="#9333ea" />
</div>
...
<div style="background:#fff; border-radius:14px; border:1px solid #e2e8f0; box-shadow:0 1px 6px rgba(0,0,0,0.05); overflow:hidden;">
    <div style="padding:14px 20px; border-bottom:1px solid #f1f5f9;">
        <span style="font-size:0.88rem; font-weight:700; color:#1e293b;">Repayment Records <span style="color:#94a3b8; font-weight:400;">({{ $repayments->total() }})</span></span>
    </div>
    <div style="overflow-x:auto;">
        <table style="width:100%; border-collapse:collapse; font-size:0.83rem;">
            <thead>
                <tr style="background:#f8fafc;">
                    @foreach(['#','Member','Samity','Loan','Amount Paid','Principal','Interest','Paid Date','Note','Actions'] as $h)
            ...
            <tbody>
                ...
                <tr style="border-top:1px solid #f1f5f9;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background=''">
                    ...
                    <td style="padding:12px 16px; font-weight:700; color:#16a34a;">৳{{ number_format($r->amount_paid, 2) }}</td>
                    <td style="padding:12px 16px; color:#0d9488; font-weight:600;">৳{{ number_format($r->principal, 2) }}</td>
                    <td style="padding:12px 16px; color:#9333ea; font-weight:600;">৳{{ number_format($r->interest, 2) }}</td>
                ...
                <tr><td colspan="10" style="padding:48px;text-align:center;color:#94a3b8;">
                    <i class="fas fa-rotate-left" style="font-size:2.5rem;opacity:.2;display:block;margin-bottom:10px;"></i>No repayment records found.
                </td></tr>
                @endforelse
            </tbody>
        </table>
...
<x-modal id="modal-create" title="Record Repayment" icon="fas fa-rotate-left">
    <form action="{{ route('repayments.store') }}" method="POST">
        ...
        <div style="display:flex; gap:10px; justify-content:flex-end; margin-top:20px; padding-top:16px; border-top:1px solid #f1f5f9;">
            ...
            <button type="submit" style="{{ $btn_primary }}"><i class="fas fa-save"></i> Save Repayment</button>
        </div>
    </form>
</x-modal>

{{-- ═══ EDIT MODAL ═══ --}}
<x-modal id="modal-edit" title="Edit Repayment" icon="fas fa-pen-to-square">
    <form id="edit-repayment-form" method="POST">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:14px;">
            <div style="grid-column:1/-1;">
                ...
                <select name="loan_id" id="er_loan" style="{{ $inp }}" required>
                    @foreach($loans as $ln)
                        <option value="{{ $ln->id }}">#{{ $ln->id }} — {{ $ln->user?->name }} (৳{{ number_format($ln->amount, 2) }})</option>
                    @endforeach
                </select>
            </div>
            <div>
        ...
        <div style="display:flex; gap:10px; justify-content:flex-end; margin-top:20px; padding-top:16px; border-top:1px solid #f1f5f9;">
            ...
            <button type="submit" style="{{ $btn_primary }}"><i class="fas fa-save"></i> Update Repayment</button>
        </div>
    </form>
</x-modal>

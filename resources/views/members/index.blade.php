...
<div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:12px; margin-bottom:20px;">
    <div>
        <h1 style="font-size:1.25rem; font-weight:800; color:#0f172a; margin:0;">Members</h1>
        <p style="font-size:0.8rem; color:#64748b; margin:3px 0 0;">Manage all registered members</p>
    </div>
...
<div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(180px,1fr)); gap:14px; margin-bottom:20px;">
    <x-stat-card label="Total Members" :value="$stats['total']"    icon="fas fa-users"        bg="#eff6ff" iconColor="#2563eb" />
    ...
    <x-stat-card label="Admins"        :value="$stats['admins']"   icon="fas fa-user-shield"  bg="#faf5ff" iconColor="#9333ea" />
</div>
...
<form method="GET" style="background:#fff; border-radius:12px; border:1px solid #e2e8f0; padding:14px 18px; margin-bottom:16px; display:flex; flex-wrap:wrap; gap:10px; align-items:flex-end;">
    <div style="flex:1; min-width:180px;">
        ...
        <div style="position:relative;">
            ...
            <input name="search" value="{{ request('search') }}" placeholder="Name, email, phone or NID..."
...
<div style="background:#fff; border-radius:14px; border:1px solid #e2e8f0; box-shadow:0 1px 6px rgba(0,0,0,0.05); overflow:hidden;">
    <div style="padding:14px 20px; border-bottom:1px solid #f1f5f9;">
        <span style="font-size:0.88rem; font-weight:700; color:#1e293b;">All Members <span style="color:#94a3b8; font-weight:400;">({{ $members->total() }})</span></span>
    </div>
    <div style="overflow-x:auto;">
        <table style="width:100%; border-collapse:collapse; font-size:0.83rem;">
            <thead>
                <tr style="background:#f8fafc;">
                    @foreach(['#','Member','Phone','NID','Role','Status','Joined','Actions'] as $h)
            ...
            <tbody>
                ...
                <tr><td colspan="8" style="padding:48px;text-align:center;color:#94a3b8;">
                    <i class="fas fa-users" style="font-size:2.5rem;opacity:.2;display:block;margin-bottom:10px;"></i>No members found.
                </td></tr>
                @endforelse
            </tbody>
        </table>
...
{{-- ═══ CREATE MODAL ═══ --}}
<x-modal id="modal-create" title="Add New Member" icon="fas fa-user-plus">
    <form action="{{ route('members.store') }}" method="POST">
        @csrf
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:14px;">
            <div>
                <label style="{{ $lbl }}">Full Name *</label>
                <input name="name" value="{{ old('name') }}" placeholder="Full name" style="{{ $inp }}" required>
            </div>
            <div>
            ...
            <div>
                <label style="{{ $lbl }}">NID</label>
                <input name="nid" value="{{ old('nid') }}" placeholder="National ID number" style="{{ $inp }}">
            </div>
            <div style="grid-column:1/-1;">
                <label style="{{ $lbl }}">Address</label>
                <input name="address" value="{{ old('address') }}" placeholder="Full address" style="{{ $inp }}">
            </div>
            <div>
                <label style="{{ $lbl }}">Role *</label>
            ...
            <div>
                ...
                <input type="password" name="password" placeholder="Min 6 characters" style="{{ $inp }}" required>
            </div>
            <div style="grid-column:1/-1;">
                <label style="{{ $lbl }}">Confirm Password *</label>
                <input type="password" name="password_confirmation" placeholder="Repeat password" style="{{ $inp }}" required>
            </div>
        </div>
        <div style="display:flex; gap:10px; justify-content:flex-end; margin-top:20px; padding-top:16px; border-top:1px solid #f1f5f9;">
            ...
            <button type="submit" style="{{ $btn_primary }}"><i class="fas fa-save"></i> Save Member</button>
        </div>
    </form>
</x-modal>

{{-- ═══ EDIT MODAL ═══ --}}
<x-modal id="modal-edit" title="Edit Member" icon="fas fa-user-pen">
    <form id="edit-member-form" method="POST">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:14px;">
            ...
            <div style="display:flex; align-items:center; gap:10px; padding-top:22px;">
                ...
                <label for="em_is_active" style="font-size:0.85rem; font-weight:600; color:#374151; cursor:pointer;">Active Account</label>
            </div>
            <div>
                <label style="{{ $lbl }}">New Password <span style="color:#94a3b8;font-weight:400;">(leave blank to keep)</span></label>
                <input type="password" name="password" placeholder="New password" style="{{ $inp }}">
            </div>
            <div>
                <label style="{{ $lbl }}">Confirm New Password</label>
                <input type="password" name="password_confirmation" placeholder="Confirm new password" style="{{ $inp }}">
            </div>
        </div>
        <div style="display:flex; gap:10px; justify-content:flex-end; margin-top:20px; padding-top:16px; border-top:1px solid #f1f5f9;">
            <button type="button" onclick="closeModal('modal-edit')" style="{{ $btn_cancel }}">Cancel</button>
            <button type="submit" style="{{ $btn_primary }}"><i class="fas fa-save"></i> Update Member</button>
        </div>
    </form>
</x-modal>

...
<div style="display:flex; align-items:center; justify-content:space-between; flex-wrap:wrap; gap:12px; margin-bottom:20px;">
    <div>
        <h1 style="font-size:1.25rem; font-weight:800; color:#0f172a; margin:0;">Deposits</h1>
        <p style="font-size:0.8rem; color:#64748b; margin:3px 0 0;">Track all member savings and deposits</p>
    </div>
...
<div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(180px,1fr)); gap:14px; margin-bottom:20px;">
    <x-stat-card label="Total Deposits" :value="'৳' . number_format($stats['total_amount'], 2)" icon="fas fa-piggy-bank"     bg="#f0fdf4" iconColor="#16a34a" />
    ...
    <x-stat-card label="Total Records"  :value="$stats['total_count']"                           icon="fas fa-receipt"        bg="#f0fdfa" iconColor="#0d9488" />
    <x-stat-card label="Pending"        :value="$stats['pending']"                               icon="fas fa-clock"          bg="#fefce8" iconColor="#ca8a04" />
</div>
...
<div style="background:#fff; border-radius:14px; border:1px solid #e2e8f0; box-shadow:0 1px 6px rgba(0,0,0,0.05); overflow:hidden;">
    <div style="padding:14px 20px; border-bottom:1px solid #f1f5f9;">
        <span style="font-size:0.88rem; font-weight:700; color:#1e293b;">Deposit Records <span style="color:#94a3b8; font-weight:400;">({{ $deposits->total() }})</span></span>
    </div>
    <div style="overflow-x:auto;">
        <table style="width:100%; border-collapse:collapse; font-size:0.83rem;">
            <thead>
                <tr style="background:#f8fafc;">
                    @foreach(['#','Receipt','Member','Samity','Amount','Date','Status','Note','Actions'] as $h)
            ...
            <tbody>
                ...
                <tr style="border-top:1px solid #f1f5f9;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background=''">
                    ...
                    <td style="padding:12px 16px; font-weight:700; color:#16a34a;">৳{{ number_format($d->amount, 2) }}</td>
                ...
                <tr><td colspan="9" style="padding:48px;text-align:center;color:#94a3b8;">
                    <i class="fas fa-piggy-bank" style="font-size:2.5rem;opacity:.2;display:block;margin-bottom:10px;"></i>No deposit records found.
                </td></tr>
                @endforelse
            </tbody>
        </table>
...
{{-- ═══ CREATE MODAL ═══ --}}
<x-modal id="modal-create" title="Record Deposit" icon="fas fa-piggy-bank">
    <form action="{{ route('deposits.store') }}" method="POST">
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:14px;">
            ...
            <div>
                ...
                <input name="receipt_number" value="{{ old('receipt_number') }}" placeholder="Auto-generated if empty" style="{{ $inp }}">
            </div>
        ...
        <div style="display:flex; gap:10px; justify-content:flex-end; margin-top:20px; padding-top:16px; border-top:1px solid #f1f5f9;">
            ...
            <button type="submit" style="{{ $btn_primary }}"><i class="fas fa-save"></i> Save Deposit</button>
        </div>
    </form>
</x-modal>

{{-- ═══ EDIT MODAL ═══ --}}
<x-modal id="modal-edit" title="Edit Deposit" icon="fas fa-pen-to-square">
    <form id="edit-deposit-form" method="POST">
        ...
        <div style="display:flex; gap:10px; justify-content:flex-end; margin-top:20px; padding-top:16px; border-top:1px solid #f1f5f9;">
            ...
            <button type="submit" style="{{ $btn_primary }}"><i class="fas fa-save"></i> Update Deposit</button>
        </div>
    </form>
</x-modal>
